...
async def handle_event(event: dict):
    ...
    from app.core.audit_service import create_audit_log_from_event

    await create_audit_log_from_event(event)


class KafkaConsumer:
    _instance: AIOKafkaConsumer | None = None
    _started: bool = False
    ...
    async def start(cls):
        ...
        if not cls._started:
            ...
            asyncio.create_task(cls._consume_events())
...
async def handle_event(event: dict):
    # Import here to avoid circular imports
    from app.services.audit_service import create_audit_log_from_event
